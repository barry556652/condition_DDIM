...
class UNet(nn.Module):
    def __init__(self, T, num_labels, ch, ch_mult, num_res_blocks, dropout):
        ...
        self.input_blocks.append(TimestepEmbedSequential(nn.Conv2d(ch, ch, 3, padding=1)))
        ...
        for i, mult in enumerate(ch_mult):
            ...
            for _ in range(num_res_blocks):
                ...
                if i in attention_levels:
                    self.downblocks.append(SpatialTransformer(now_ch, n_heads, tf_layers, d_cond))
                self.downblocks.append(TimestepEmbedSequential(*self.downblocks)
                chs.append(now_ch)
            if i != len(ch_mult) - 1:
        ...
        self.middleblocks = nn.ModuleList([
            ...
            SpatialTransformer(now_ch, n_heads, tf_layers, d_cond),
            ResBlock(now_ch, now_ch, tdim, dropout, attn=False),
        ])
        ...
        for i, mult in reversed(list(enumerate(ch_mult))):
            ...
            for _ in range(num_res_blocks + 1):
                ...
                now_ch = out_ch
                if i in attention_levels:
                    self.upblocks.append(SpatialTransformer(now_ch, n_heads, tf_layers, d_cond))

            if i != 0:
                self.upblocks.append(UpSample(now_ch))
            self.upblocks.append(TimestepEmbedSequential(*self.upblocks))
        assert len(chs) == 0
    ...
    def forward(self, x, t, labels, labels_1):
        ...
        cemb = self.cond_embedding(labels)[:, None, :]
        cemb1 = self.com_embedding(labels_1)[:, None, :]
        cond = torch.cat((cemb, cemb1), dim=1)
        # Downsampling
        h = self.head(x)
        hs = [h]
